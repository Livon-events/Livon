...
### Area
- Required. Single-select from the Areas within a City (City dropdown → Area chips, same data as the header's location picker — see docs/FR/location-toggle.md — but a fully separate field with its own state). Cities currently include Maseru and Berea.
