...
```
get_public_connections_count(p_user_id uuid) RETURNS integer
```
`SQL`, `STABLE`, `SECURITY DEFINER`, `search_path = public`

Added to back the "N connections" line on another user's profile page
(`/profile/[userId]`). Trivial `count(*)` over `connections` for a given
user (`status = 'accepted'`, either `requester_id` or `receiver_id`
matching). Same "count-only, safe to expose" exception already used by
`event_going_count` — `connections`' own RLS only lets a client see rows
where *they themselves* are a party, so a stranger's total needs this
SECURITY DEFINER wrapper; it hands back nothing but the integer, never
raw rows, so it's safe to grant to `anon`/`authenticated` even though the
underlying table is otherwise locked down to "own rows only."
...
grant execute on function public.get_public_connections_count(uuid) to anon, authenticated;
```

Not yet run against the live Supabase instance — needs to be pasted into
the SQL editor once (same workflow as prior schema/RLS changes).
