  ...
  return (
    <div className="flex items-center gap-3.5 bg-[#17181A] rounded-2xl px-3 py-2.5">
      {/* Action button is a sibling, not nested inside this Link, so it
          ...
          a navigation. */}
      <Link
        href={actionLabel === "manage" ? `/events/${event.id}/manage` : `/events/${event.id}`}
      ...
      >
        <div
          ...
          style={event.imageUrl ? { backgroundImage: safeBackgroundImage(event.imageUrl) } : undefined}
        />
      ...
      <button
        ...
        title={actionLabel}
        className="flex items-center justify-center border-none rounded-[10px] w-10 h-10 bg-[#FFF335] text-[#191919] cursor-pointer flex-shrink-0"
      >
